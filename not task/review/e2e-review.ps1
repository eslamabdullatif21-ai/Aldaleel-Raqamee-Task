...
function Login([string]$Email, [string]$Password = 'Reviewer123!') {
...
$agent1 = [pscustomobject]@{ id='e6cd6def-7b2b-4bc4-b965-8b1376479e60'; email='agent@example.com' }
$agent2 = [pscustomobject]@{ id='33333333-3333-3333-3333-333333333333'; email='agent2@example.com' }

$r = Invoke-Api POST '/api/auth/register' @{ email='review.agent@example.com'; password='Reviewer123!'; name='Review Agent'; role='AGENT' }
Check 'Public agent registration is forbidden' ($r.Status -eq 403 -and $r.Json.message -eq 'Public registration is limited to customer accounts') "HTTP $($r.Status), message=$($r.Json.message)"
...
$agent1Token = Login $agent1.email 'Agent123!'
$agent2Token = Login $agent2.email 'Agent123!'
...
Check 'New ticket is absent from agent list before claim' ($r.Status -eq 200 -and $ticket1.id -notin @($r.Json.content.id)) "HTTP $($r.Status), total seeded assignments=$($r.Json.page.totalElements)"
...
$r = Invoke-Api PATCH "/api/tickets/$($ticket2.id)/assign" @{} $agent1Token
Check 'Agent self-claims before handoff' ($r.Status -eq 200 -and $r.Json.assignedAgentId -eq $agent1.id) "HTTP $($r.Status), agent=$($r.Json.assignedAgentId)"
$r = Invoke-Api PATCH "/api/tickets/$($ticket2.id)/assign" @{agentId=$agent2.id} $agent1Token
Check 'Assigned agent can hand off to a named agent' ($r.Status -eq 200 -and $r.Json.assignedAgentId -eq $agent2.id) "HTTP $($r.Status), agent=$($r.Json.assignedAgentId)"
